...
/**
 * Created by zhengjy on 2016/11/28.
 */

public class PolylineView extends View {
    private static final float LEFT = 1 / 16F;
    private static final float TOP = 1 / 16F;
    private static final float RIGHT = 15 / 16F;
    private static final float BOTTOM = 7 / 8F;// 网格区域相对位置
    private static final float TIME_X = 3 / 32F;
    private static final float TIME_Y = 1 / 16F;
    private static final float MONEY_X = 31 / 32F;
    private static final float MONEY_Y = 15 / 16F;// 文字坐标相对位置
    private static final float TEXT_SIGN = 1 / 32F;// 文字相对大小
    private static final float THICK_LINE_WIDTH = 1 / 128F;
    private static final float THIN_LINE_WIDTH = 1 / 512F;// 粗线和细线相对大小

    private TextPaint mTextPaint;// 文字画笔
    private Paint mLinePaint, mPointPaint;// 线条画笔和点画笔
    ...
    private String mSignX, mSignY;// 设置X和Y坐标分别表示什么的文字
    ...
    private void drawPolyline(Canvas canvas) {
        ...
        for (int i = 0; i < mPointFs.size(); i++) {
            // 计算x坐标
            float x = mCanvas.getWidth() / maxX * mPointFs.get(i).x;

            // 计算y坐标
            float y = mCanvas.getHeight() / maxY * mPointFs.get(i).y;
    ...
    private void drawLines(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            if (maxX < mPointFs.get(i).x) {
                maxX = mPointFs.get(i).x;
            }
        }
        ...
        maxY = 0;
        for (int i = 0; i < count; i++) {
            if (maxY < mPointFs.get(i).y) {
                maxY = mPointFs.get(i).y;
            }
        }
    ...
    private void drawSign(Canvas canvas) {
        ...
        canvas.drawText(null == mSignY ? "y" : mSignY, textY_X, textY_Y, mTextPaint);

        // 绘制横轴标识文字
        mTextPaint.setTextAlign(Paint.Align.RIGHT);
        canvas.drawText(null == mSignX ? "x" : mSignX, textX_X, textX_Y, mTextPaint);
